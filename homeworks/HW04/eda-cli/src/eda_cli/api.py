...
# ---------- /quality-flags-from-csv: качество данных через CSV и EDA-логику ----------

@app.post(
    "/quality-flags-from-csv",
    response_model=QualityFlagsResponse,
    tags=["quality-flags"],
    summary="Вывод флагов качества данных по CSV-файлу с использованием EDA-ядра",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    и возвращает набор флагов качества данных.
    """
    ...
    for item in flags_all:
        flags_all[item] = str(flags_all[item])
