...
class DatasetSummary:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "constant_columns_count": self.constant_columns_count,
            "max_zero_share": self.max_zero_share,
        }
...
def summarize_dataset(
    ...
    for name in df.columns:
        ...
        if is_numeric and non_null > 0:
            ...
            zero_count = (s == 0).sum()
            current_zero_share = zero_count / non_null
            if current_zero_share > max_zero_share:
                max_zero_share = current_zero_share
    ...
    return DatasetSummary(n_rows=n_rows, n_cols=n_cols, columns=columns, constant_columns_count=constant_columns_count, max_zero_share=max_zero_share)
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_constant_columns"] = summary.constant_columns_count > 0
    flags["has_many_zero_values"] = summary.max_zero_share > 0.8
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if summary.constant_columns_count > 0:
        score -= 0.15
    if summary.max_zero_share > 0.8:
        score -= 0.1
